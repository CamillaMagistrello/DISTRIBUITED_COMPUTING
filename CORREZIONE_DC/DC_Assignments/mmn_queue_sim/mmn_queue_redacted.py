...
import csv
import os
from random import expovariate, randrange,  seed,sample
...
# class to run multiple simulation of MMN queue
# with different configurations
class MMNImplSim:

    def __init__(self, total_sim_params, max_t, file, plot_file):
        simulations = []

        # map to retrieve unique values for each parameter
        change_var_counter = {"lambd": [],
                              "mu": [],
                              "n": [],
                              "d": [],
                              "shape": []}
        ...
        for i, sim_params in enumerate(total_sim_params):
            simulations.append(MMNImpl(sim_params["lambd"],
                                       sim_params["mu"],
                                       sim_params["n"],
                                       sim_params["d"],
                                       sim_params["shape"]))

            # registering values for config params
            for k, v in sim_params.items():
                if v not in change_var_counter[k]:
                    change_var_counter[k].append(v)

        title = "Supermarket model"
        text = ""
        legend_labels = []  # list of tags to be put into legend

        # determine if a specific writing will be in the legend or in the textbox
        for key in change_var_counter.keys():
            if key == "shape":  # only shape used is exponential distribution (shape = 1)
                continue

            if len(change_var_counter.get(key)) == 1:  # if this value is unique, goes in the box
                text = text + key + " = " + str(change_var_counter[key][0]) + "\n"
            else:  # otherwise all its values will be put in the legend
                for el in change_var_counter[key]:
                    legend_labels.append(key + " = " + str(el))

        # running simulations and plotting results
        for i, sim in enumerate(simulations):
            sim.simulate(max_t, total_sim_params[i]["shape"], file, None)
            total_qdistr_len = len(sim.queue_len_distr.keys()) * sim.n
            total_lengths = []

            # retrieving all values of queue length
            for l in sim.queue_len_distr.values():
                for el in l:
                    total_lengths.append(el)
            counter = Counter(total_lengths)  # creating and sorting by key a counter object
            sorted_per_value_counter = sorted(counter.items())
            qlen_less = 0  # number of queues with lower size
            qdistr_plot = {}  # map queue_length - fraction of queues with at least that size

            # calculating the complementary of CDF (P(length >= val))
            for q_len, quantity in sorted_per_value_counter:
                qdistr_plot[q_len] = 1.0 - (qlen_less / total_qdistr_len)
                qlen_less += quantity
            plt.plot(qdistr_plot.keys(), qdistr_plot.values(), label=legend_labels[i], color=colors[i])

        # textbox properties
        if text != "":
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
            plt.gcf().text(0.70, 0.86, text, fontsize=8, bbox=props)
            ...
            with open(file, 'a', newline='') as f:
                ...
                for system in self.systems.values():
                    writer.writerow([lambd, mu, max_t, system.quantum, d, n,
                                     self.quantum_times[system.quantum], shape])
...
# class to run multiple round robin simulations
# with different configurations
class MMNRoundRobinMultipleSim:
    ...
    def __init__(self, total_sim_params, max_t, file, plot_file):
        ...
        # create and launch simulation objects
        for i, sim_params in enumerate(total_sim_params):
            simulations.append(MMNRoundRobinSim(sim_params["lambd"],
                                                sim_params["mu"],
                                                sim_params["n"],
                                                sim_params["d"],
                                                max_t,
        ...
        for j, sim in enumerate(simulations):
            plt.plot(sim.quantum_times.keys(), sim.quantum_times.values(),
                     label=legend_labels[j], color=colors[j])
        ...
        # plt.axis([0, 50, 0, 50])
        plt.xlabel("Quantum of time")
        plt.ylabel("Average time spent by a job in the system")
        plt.legend(loc=0)
        plt.grid()
        plt.show()
        if plot_file is not None:
...
def main():
    ...
    parser.add_argument('--rr', type=int, default=0)
    parser.add_argument('--plots-dir', type=str, default="./fixed_plot")
    ...
    if args.rr == 1:
        ...
        tot_sim_params = [
            {"lambd": 0.5, "mu": 1.0, "d": 5, "n": 10, "shape": 0.5},
            {"lambd": 0.5, "mu": 1.0, "d": 5, "n": 10, "shape": 1},
            {"lambd": 0.5, "mu": 1.0, "d": 5, "n": 10, "shape": 1.5},
            {"lambd": 0.5, "mu": 1.0, "d": 5, "n": 10, "shape": 3},
            {"lambd": 0.5, "mu": 1.0, "d": 5, "n": 10, "shape": 3.75}
        ]
        my_simulation = MMNRoundRobinMultipleSim(tot_sim_params, args.max_t, args.csv_rr,
                                                 get_next_plot_name(args.plots_dir + "/extended"))
        print("Round Robin simulation finished")
    else:
        print("Starting new Supermarket simulation")
        tot_sim_params = [
            {"lambd": 0.5, "mu": 1.0, "d": 1, "n": 100, "shape": weibull_shape},
            {"lambd": 0.9, "mu": 1.0, "d": 1, "n": 100, "shape": weibull_shape},
            {"lambd": 0.95, "mu": 1.0, "d": 1, "n": 100, "shape": weibull_shape},
            {"lambd": 0.99, "mu": 1.0, "d": 1, "n": 100, "shape": weibull_shape}
        ]
        sim = MMNImplSim(tot_sim_params, args.max_t, args.csv, get_next_plot_name(args.plots_dir + "/base"))
