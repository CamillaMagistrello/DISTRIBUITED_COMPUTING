...
class Backup(Simulation):
    ...
    # registers the bandwidth waste
    def register_bw_waste(self, time):

        # we take into account only those nodes tht are online and that are uploading something
        l_up = [node.available_bw_upload
                                   for node in self.nodes if node.online
                                   and len(node.current_uploads.values()) > 0]

        # we take into account only those nodes tht are online and that are downloading something
        l_down = [node.available_bw_download
                                   for node in self.nodes if node.online
                                   and len(node.current_downloads.values()) > 0]

        # registering the average upload/download bandwidth waste for given time
    ...
    def scheduleTransfer(self, uploader: 'Node', downloader: 'Node', block_id: int, restore: bool):
        ...
        # the block size depends on the direction of the transfer: if it is a restore (from uploader to downloader), we take te downloader's block size; otherwise, if it is a backup,
        # we use the uploader's one
        block_size = downloader.block_size if restore else uploader.block_size

        # ensure that both uploader nd downloader hve some bandwidth available otherwise, it is an error
        assert uploader.available_bw_upload > 0
        assert downloader.available_bw_download > 0

        # we take the slowest speed b/w the available bandwidths
        speed = min(uploader.available_bw_upload, downloader.available_bw_download)

        # calculating the time needed for the transfer
        delay = block_size / speed

        # updating the  available bandwidth for uploader and downloader
        uploader.available_bw_upload -= speed
        downloader.available_bw_download -= speed

        # distinguishing the restore from backup transfer
        if restore:
...
# class to perform at the same time simulation with single uploads/download and parallel ones that produces plots to compare results
class BackupSim:
    ...
    def __init__(self, max_t, nodes):
        ...
        # creating nd launching different simulations with the same events occurrence
        ...
        # in order to analyze the distribution of the wasted bandwidth the Cumulative Distribution Function (cdf) is calculated and plotted
        normal_sim_up_ecdf = self.ecdf_dict(normal_sim.up_bw_wasted.values())
        normal_sim_dw_ecdf = self.ecdf_dict(normal_sim.dw_bw_wasted.values())
        extended_sim_up_ecdf = self.ecdf_dict(extended_sim.up_bw_wasted.values())
        extended_sim_dw_ecdf = self.ecdf_dict(extended_sim.dw_bw_wasted.values())

        # plotting results
        fig, axs = plt.subplots(2, 2)
        plt.suptitle("Wasted bandwidth distribution")
        axs[0, 0].plot(normal_sim_up_ecdf.keys(), normal_sim_up_ecdf.values())
        axs[0, 0].set_title('Single upload bandwidth wasted cdf')
        axs[0, 1].plot(normal_sim_dw_ecdf.keys(), normal_sim_dw_ecdf.values(), 'tab:orange')
        axs[0, 1].set_title('Single download bandwidth wasted cdf')
        axs[1, 0].plot(extended_sim_up_ecdf.keys(), extended_sim_up_ecdf.values(), 'tab:green')
        axs[1, 0].set_title('Parallel upload bandwidth wasted cdf')
        axs[1, 1].plot(extended_sim_dw_ecdf.keys(), extended_sim_dw_ecdf.values(), 'tab:red')
        axs[1, 1].set_title('Parallel download bandwidth wasted cdf')

        for ax in axs.flat:
            ax.set(xlabel='Wasted bandwidth (MB)', ylabel='cdf')

        # building the textboxes to show config params
        text = ""
        text2 = ""
        t1 = False
    ...
    # utility that returns a list of random values
    # having the specified mean generated with exponential distribution
    ...
    # utility function that given a list of elements returns the associated
    # CDF dict where keys = {x | x in list} and
    # foreach k in keys.CDF[k] = P(list_element <= k)
    def ecdf_dict(self, arr):
        length = len(arr)  # saving list cardiniality
        counter = Counter(arr)  # creating counter object
        sorted_occ_list = sorted(counter.items())  # sort the counter by key
        ecdf_waste = {}  # map for x - CDF(x)
        previous_vals = 0  # cumulative sum of previous elements' occurrences
        for waste, occurrence in sorted_occ_list:
            previous_vals += occurrence
            ecdf_waste[waste] = previous_vals / length

        # if my distribution is constant, in order to have at least two points
        # to construct a graph, a fake key with the same value will be added
        if len(list(ecdf_waste.keys())) == 1:
            new_key = list(ecdf_waste.keys())[0] + pow(10, -3)
            ecdf_waste[new_key] = list(ecdf_waste.values())[0]
        return ecdf_waste
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        for peer, block_id in self.remote_blocks_held.items():
            ...
            if peer.online and constraint and not peer.local_blocks[block_id]:
                sim.scheduleTransfer(self, peer, block_id, True)
        ...
        for peer in sim.nodes:
            ...
            if (peer is not self and peer.online and peer not in remote_owners and constraint
                ...
                sim.scheduleTransfer(self, peer, block_id, False)  # scheduling the uploading of the block by this node
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
            ...
            if not held_locally and peer is not None and peer.online and constraint:
                sim.scheduleTransfer(peer, self, block_id, True)
        ...
        for peer in sim.nodes:
            ...
            if (peer is not self and peer.online and constraint and peer not in
                ...
                if block_id is not None:
                    sim.scheduleTransfer(peer, self, block_id, False)
                    return True
...
class TransferComplete(Event):
    ...
    def process(self, sim: Backup):
        ...
        # this transfer was canceled, so ignoring this event
        if self.canceled:
            return
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self):
        ...
        owner.local_blocks[self.block_id] = True  # updating the blocks held locally
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks so we have all of them then
