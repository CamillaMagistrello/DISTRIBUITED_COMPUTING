...
class Backup(Simulation): # Backup simulation.
    ...
    def __init__(self, nodes: List['Node']):
        super().__init__()  # call the __init_ method of parent class
        self.nodes = nodes
    ...
    def schedule_transfer(self, uploader: 'Node', downloader: 'Node', block_id: int, restore: bool):
        ...
        # self.log_info(f"scheduled {event._class.name_} from {uploader} to {downloader}"
...
class Node:
    ...
    # def _init_(self, description, n, k, ...):
    #     self.n = n
    #     self.k = k
    #     ...
    #     self._post_init_()  # if the method exists
    ...
    def __post_init__(self):
        ...
        self.online: bool = False # whether this node is online. All nodes start offline.
        self.failed: bool = False# whether this node is currently under repairs. All nodes are ok at start.
        self.block_size: int = self.data_size // self.k if self.k > 0 else 0 # size of each block
        self.free_space: int = self.storage_size - self.block_size * self.n # amount of free space for others' data -- note we always leave enough space for our n blocks
        ...
        self.remote_blocks_held: dict[Node, int] = {} # (owner -> block_id) mapping for remote blocks stored
        self.current_upload: Optional[TransferComplete] = None # current uploads and downloads, stored as a reference to the relative TransferComplete event
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:
            owner.local_blocks = [True for _ in range(len(owner.local_blocks))]
